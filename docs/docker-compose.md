...
| `GO2RTC_FFMPEG_LOG` | `error`                        | go2rtc's own ffmpeg-subprocess log level. `error` (go2rtc's default) is quiet in normal operation; set to `debug` and restart to see why a specific stream won't play — go2rtc.yaml is regenerated fresh at every boot, so this can't be set by hand-editing the file                                                                                           |
