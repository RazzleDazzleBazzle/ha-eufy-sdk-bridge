...
| `BRIDGE_STREAM_FPS`          | `15`                           | declared frame rate go2rtc's ffmpeg pull uses to derive real timestamps for a camera's raw, untimed P2P feed — the wire itself carries no fps. 15 matches what real Eufy cameras were consistently measured at; only raise/lower this if a specific model is confirmed to run at a different real rate (a wrong value shows up as visibly wrong playback speed)                                                                                                                                                                                                                                                                          |
| `GO2RTC_FFMPEG_LOG`          | `error`                        | go2rtc's ffmpeg-subprocess verbosity. Set to `debug` **together with** `GO2RTC_LOG_LEVEL` (below) to actually see it — this alone controls how much ffmpeg produces, not whether go2rtc forwards it into the visible log                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `GO2RTC_LOG_LEVEL`           | unset (go2rtc's own default)   | go2rtc's global internal log level — the fallback every module without its own override uses, including the one that decides whether to forward ffmpeg's stderr at all. Set to `debug` alongside `GO2RTC_FFMPEG_LOG=debug` to diagnose a stream that won't play; go2rtc.yaml is regenerated fresh at every boot, so neither can be set by hand-editing the file                                                                                                                                                                                                                                                                          |
| `STREAM_RECONNECT_GRACE_MS`  | `30000` (30s)                  | how long a `/stream` feed is kept open (drained, not destroyed) after its consumer disconnects, so a quick reconnect reuses the already-warm P2P session instead of paying a fresh handshake. Exists because Home Assistant's own RTSP client hard-codes a 5s read timeout with no override hook — a camera whose P2P handshake is slower than that (some models measured ~10-11s) always fails HA's _first_ attempt; HA auto-retries ~10s later, and reusing the warm feed on that retry lands well inside its 5s window instead of racing the handshake again from cold. `0` disables (tears down immediately, the previous behaviour) |
| `SNAPSHOT_WARM_INTERVAL_MIN` | unset (off)                    | periodically refresh a cache for `/snapshot/<sn>` every N minutes instead of every request paying for a live P2P wake — Apple Home (or anything else) decides on its own when to ask for a tile snapshot, so without this a burst of those requests is a burst of live wakes. Piggybacks on a fresher "Last event" push-driven image when one exists, and skips a camera someone's actively streaming right now. Unset/`0` keeps today's behaviour (always live, no caching)                                                                                                                                                             |
| `SNAPSHOT_WARM_STAGGER_MS`   | `5000` (5s)                    | gap between each camera's turn within one sweep, so a multi-camera refresh doesn't hit every station's shared P2P slot at once. Only matters when `SNAPSHOT_WARM_INTERVAL_MIN` is set                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `SNAPSHOT_WARM_DEVICES`      | unset (every camera)           | comma-separated serials to limit the sweep to — e.g. `T8160P.../T8210P...`. Only matters when `SNAPSHOT_WARM_INTERVAL_MIN` is set                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `SNAPSHOT_NO_LIVE_DEVICES`   | unset (no restriction)         | comma-separated serials that must NEVER pay for a live P2P pull for a snapshot at all — not on the periodic sweep, and not as that camera's own `/snapshot` fallback either. `/snapshot` for one of these only ever serves whatever's already sitting around: the warm-up cache (if it landed one via the free piggyback), Eufy's own retained picture, or last resort this bridge's own persisted "Last event" cover — never a fresh capture. Independent of `SNAPSHOT_WARM_INTERVAL_MIN`; works even with the sweep entirely off                                                                                                       |
